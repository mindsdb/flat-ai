...
class FlatAI:
    ...
    def get_code(self, prompt: str, **kwargs) -> str:
        """Get a code response from the LLM returned as a PythonCodeObject"""
        instruction_message = INSTRUCTION_MESSAGE
        plain_llm_response = self.get_string(prompt, instruction_message=instruction_message, **kwargs)
        code_object = self.generate_object(PythonCodeObject, text_with_code=plain_llm_response)
        return code_object
    
    def get_code_as_stream(self, prompt: str, **kwargs) -> str:
        """Get a code response from the LLM returned as stream of chunks, last chunk is a PythonCodeObject"""

        instruction_message = INSTRUCTION_MESSAGE
        plain_llm_response = ''    
        for chunk in self.get_stream(prompt, instruction_message=instruction_message, **kwargs):
            plain_llm_response += chunk
            yield chunk
        code_object = self.generate_object(PythonCodeObject, text_with_code=plain_llm_response)
        yield code_object
